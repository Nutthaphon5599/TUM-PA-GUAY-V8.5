...
function render(){
  ...
  const payByOrder={};payments.forEach(p=>payByOrder[p.order_id]=p.method);$("#rows").innerHTML=orders.map(o=>`<tr><td data-label="ວັນທີ">${new Date(o.closed_at||o.created_at).toLocaleString("lo-LA")}</td><td data-label="Bill">${o.order_number}</td><td data-label="ຕູບ">${o.table_number||"Takeaway"}</td><td data-label="Subtotal">${money(o.subtotal)}</td><td data-label="Discount">${money(o.discount)}</td><td data-label="VAT">${money(o.vat_amount)}</td><td data-label="Total"><b>${money(o.grand_total)}</b></td><td data-label="Payment">${payByOrder[o.id]||"-"}</td></tr>`).join("")||'<tr><td colspan="8" class="empty">ບໍ່ພົບຂໍ້ມູນ</td></tr>';$("#resultInfo").textContent=`${orders.length.toLocaleString()} bills • limit 5,000`;
}
