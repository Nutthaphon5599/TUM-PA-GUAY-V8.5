...
function resetOrder(confirmFirst=false){if(confirmFirst&&cart.length&&!confirm('ເປີດບິນໃໝ່ ແລະ ລ້າງລາຍການປັດຈຸບັນ?'))return;releaseTableLock().catch(console.warn);cart=[];currentOrder=null;$('#orderNote').value='';$('#discount').value=0;$('#vatRate').value=restaurantSettings.vat_rate;updateOrderBadge();renderCart()}
...
function showReceipt(order,t,mode='paid'){if(window.innerWidth<=1180)setCartOpen(false);const pending=mode==='pending';$('#rReceiptTitle').textContent=pending?'CHECK BILL / ໃບກວດສອບລາຍການ':'RECEIPT / ໃບບິນຮັບເງິນ';$('#rPendingNotice').hidden=!pending;$('#rOrderNo').textContent=order.order_number;$('#rDate').textContent=new Date(order.closed_at||Date.now()).toLocaleString();$('#rTable').textContent=order.table_number||'Takeaway';$('#rItems').innerHTML=cart.map(x=>`<tr><td>${x.item_name}</td><td>${x.quantity}</td><td>${money(x.unit_price*x.quantity)}</td></tr>`).join('');$('#rSubtotal').textContent=money(t.subtotal);$('#rDiscount').textContent=money(t.discount);$('#rVat').textContent=money(t.vat);$('#rTotal').textContent=money(t.grand);$('#receipt').dataset.mode=mode;$('#receipt').hidden=false;document.body.style.overflow='hidden'}
